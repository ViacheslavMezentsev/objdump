...
namespace objdump.Instructions {
    ...
    // STD - Store Indirect From Register to Data Space using Index Z.
    // 10q0 qq1r rrrr 0qqq
    public class stdz: IInstruction {

        public OpInfo info;
        public OpInfo OpInfo { get { return info; } }

        public stdz() { info = new OpInfo( "STD", "Store Indirect From Register to Data Space using Index Z",
            new Regex( @"10\d0\d\d1\d{5}0\d{3}", RegexOptions.Compiled ) ); }
        ...
        public string Disassemble( List< Record > list, ref int counter ) {
            ...
            try {
                ...
                // Параметры.
                op += String.Format( "Z+{1}, R{0}", q, Rr ).PadRight( Program.CommentsPad, ' ' );
